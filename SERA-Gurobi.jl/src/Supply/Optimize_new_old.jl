...
function makeModel_new(network::Network,
    ...
    Xpress_optimizer = optimizer_with_attributes(Xpress.Optimizer,
                                                "MIPRELSTOP" => 1e-10,  # newton-barrier tolerances
                                                "BARGAPSTOP" => 1e-10,
                                                "BARDUALSTOP" => 1e-10,
                                                "BARPRIMALSTOP" => 1e-10,
                                                "MATRIXTOL" => 1e-10,
                                                "BARORDER" => 1,        #  Cholesky factorization in the Newton-Barrier
                                                "OUTPUTLOG" => optimization_log,
                                                "DEFAULTALG" => 4)    #newton barrier method is used
                                ...
                                if s in pathway_extended_storage_stages[p]
                                    ...
                                    for y in total_year_set
                                        for l in link_tot[n]
                                            ...
                                            if technology in maximum_capacity_technologies
                                                @constraint(model, pathway_stage_flow[n, l, p, s, y, t] <= maximum_capacity_constraints[technology] * periods[t[2]])
                                            end
                                        end
                                    end
